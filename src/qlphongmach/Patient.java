 ...
 * @author sieus
 */
public class Patient extends Connect_db{
    public int Insert(String[] stringsSQL)
    {
        int rowsInserted = 0;
        String sql = "INSERT INTO BENHNHAN( MaBN, TenBN, NgaySinh, GT, DiaChi,BaoHiem ) VALUES(?,?,?,?,?,?)";
    ...
    {
        int rowsInserted = 0;
        String sql = "UPDATE BENHNHAN SET TenBN = ?, NgaySinh = ?, GT = ?, DiaChi = ?,BaoHiem = ? WHERE MaBN =?";
        PreparedStatement statement;    
        try {
             statement = con.prepareStatement(sql);
            statement.setString(1, stringsSQL[0]);
    ...
    public ResultSet getDataBN (String stringSQL)
    {
        ResultSet rs = null;
        PreparedStatement statement; 
            // tạo đường dẫn kết nối database
        try {
            statement = con.prepareStatement("SELECT * FROM BENHNHAN WHERE MaBN = ? ");
